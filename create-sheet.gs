 ...
 *  4. In the dropdown next to Run (▶), select "buildSheet"
 *  5. Click Run — approve permissions when prompted
 *  6. Check the Execution log (View → Logs) to confirm all 9 tabs wrote OK
 *  7. Go back to the sheet — all tabs will be populated
 *  8. File → Share → "Anyone with the link" → Viewer
 *  9. Copy the Sheet ID from the URL and paste into index.html
 */
...
function buildSheet() {
  ...
  function write(tabName, data) {
    try {
      const sheet = getOrCreate(tabName);
      sheet.clearContents();
  ...
  write('config', [
    ...
    ['closing_headline', 'Ready to shape the space in-between.'],
    ...
    ['footer_contact',   'hello@groundreview.com\n17322 Murphy Avenue\nIrvine, CA 92614\nwww.groundreview.com'],
  ]);
  ...
  write('projects', [
    ...
    [
      'Smoke & Fire / Villains Brewery', 'Anaheim & Long Beach, California',
    ...
    [
      'Rye Goods Bakery', 'Newport Beach, Laguna Beach & Tustin, CA',
  ...
  write('workplan', [
    ...
    [
      ...
      'Competitive Review','Regional event programming analysis — what is missing in the market',
    ],
    [
  ...
  Logger.log('🎉 All done! Share the sheet and paste the ID into index.html.');
}
